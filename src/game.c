...
//interrupt reading variables
volatile int game_state = 0; //game inactive, waiting for penny

//function pointers
int[] commands =
int flip(int flipped, int placeholder){
	if(flipped){
...
void main(void){
	...
	while(1){
		switch(game_state){
			case INACTIVE:
				break;
				//not sure... do something with lights?
			case PREGAME:
				break;
				//light everything up
			case PLAYING:
				...
				if(new_command){
					...
					//execute function pointed to by input
					//if function does not exist, do nothing
...
void __attribute__((interrupt, no_auto_psv)) _INT3Interrupt(void){//penny insertion interrupt
	game_state = PREGAME;
